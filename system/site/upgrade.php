...
<html>
  ...
  <body class="page-header-fixed layout-fluid bg-ios-gray " >  
	...
	?>
		...
		$featured=null;
		$color=null;
		$package_query=mysqli_query($connect,"SELECT * FROM packages WHERE package_id>=1 ORDER BY package_id ASC");
		while($package=mysqli_fetch_array($package_query)){
		++$i;
		$package_id=$package['package_id'];
		$pv_upgrade=$me['member_pv']+0;
		if($package_id>$me_package['package_id']){
			$pv_total=$package['package_pv']-$me_package['package_pv'];
		...
		if($package_id==$me_package['package_id']){
			$color="blue";
			$featured="featured"; 
		}elseif($package_id<$me_package['package_id']){
			$color="";
			$featured=null; 
		}else{ 
			$color="";
		...
		?>
			<div class="col-sm-3 <?php echo $featured;?>">
				<div class="widget-container fluid-height list <?php echo $color;?>">
					<div class="widget-content padded text-center">
						...
						?>							
								...
								<h3><?php echo $package['package_name'];?> <small><a class="btn btn-success">Upgrade Now</a></small></h3>

						<?php
						}else{
						?>
							<h1>
								<?php echo $pv_total-$pv_upgrade;?> PV
							</h1>
							<h3><?php echo $package['package_name'];?>
							</h3>
						<?php
						}
						?>
						<ul class="text-left">
							<li>
							  Price return : $<?php echo $package['package_price_return'];?> (<?php echo number_format($package['package_price_return']*35);?> Baht)
							</li>
							<li>
							  Price value : <?php echo $package['package_pv'];?> PV
							</li>
							<li>
							  Direct bonus : <?php echo $package['package_direct_percent'];?> %
							</li>
							<li>
							  Cycle bonus : <?php echo $package['package_cycle_percent'];?> %
							</li>
							<li>
							  Max income/day : $<?php echo $package['package_cycle_maximum'];?> (<?php echo number_format($package['package_cycle_maximum']*35);?> Baht)
							</li>
							<li>
							  Autoship : $<?php echo $package['package_autoship_price'];?> (<?php echo $package['package_autoship_price']*35;?> Baht)
							</li>
